...
Jeb.prototype.update = function () {
	...
	if (closestDistance < 20) {
		...
		this.game.money.innerHTML = parseInt(this.game.money.innerHTML) + 1;
...
AM.downloadAll(function () {
	...
	gameEngine.money = document.getElementById("money");
...
socket.emit("save", { studentname: "Katie Alexander", statename: "aState", data: "Goodbye World" });
socket.emit("load", { studentname: "Katie Alexander", statename: "aState" });
...
window.onload = function () {
    ...
    var dol= document.getElementById("$").innerHTML;

    socket.on("ping", function (ping) {
        console.log("ping");
        if ( document.getElementById("money").innerHTML > money ) {
			money = document.getElementById("money").innerHTML;
			socket.emit("send", { monColl: dol + money, jebCoords: jebX + "," + jebY });
		}
        socket.emit("pong");
    });
    ...
    //money.innerHTML.onchange = function() {
        //if (e.keyCode === 13) {
        //console.log("i am inside this");
            //var doll = dol.value;
            //var totMon = money.value;
            //console.log("message sent " + doll + totMon);
            //socket.emit("send", { message: doll, username: totMon });
            //field.value = "";
        //}
    //};
    console.log("after");
